...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'update_event_type') {
        ...
        if (!$eventType || !$displayName) {
            ...
            try {
                $stmt = $db->prepare("UPDATE event_type_config SET display_name = ?, description = ?, emoji = ? WHERE event_type = ?");
                $stmt->execute([$displayName, $description, $emoji, $eventType]);
        ...
        if (!$eventType || !$displayName) {
            ...
            try {
                $stmt = $db->prepare("INSERT INTO event_type_config (event_type, display_name, description, emoji) VALUES (?, ?, ?, ?)");
                $stmt->execute([$eventType, $displayName, $description, $emoji]);
                $success = "Event type '{$displayName}' created successfully.";
            } catch (Exception $e) {
                $error = 'Event type may already exist: ' . $e->getMessage();
            }
        }
    }
}

$stmt = $db->prepare("SELECT * FROM event_type_config ORDER BY event_type ASC");
$stmt->execute();
...
<style>
    ...
    input[type="text"], textarea { width: 100%; padding: 8px; margin-top: 4px; box-sizing: border-box; border: 1px solid #ddd; border-radius: 4px; }
...
<body>
    <div class="container">
        ...
        <div class="panel">
            <h2>Event Types</h2>
            <p>Configure event types for score entry.</p>
            ...
            <div class="event-type-row">
                <h3><?php if (!empty($et['emoji'])): ?><span class="emoji"><?php echo htmlspecialchars($et['emoji']); ?></span><?php endif; ?><?php echo htmlspecialchars($et['display_name']); ?></h3>
                <p><strong>Code:</strong> <code><?php echo htmlspecialchars($et['event_type']); ?></code></p>
                <?php if (!empty($et['description'])): ?><p><strong>Description:</strong> <?php echo htmlspecialchars($et['description']); ?></p><?php endif; ?>
